...
//! constructor creating an irrklang vec3d from an irrlicht vector.
  ...
  //! returns if this vector equalsfloat the other one, taking floating point
  //! rounding errors into account
  bool equals(const vec3d<T> &other) {
    return equalsfloat(X, other.X) && equalsfloat(Y, other.Y) &&
  ...
  //! Returns if this vector interpreted as a point is on a line between two
  //! other points.
  ...
  vec3d<T> getInterpolated(const vec3d<T> &other, ik_f32 d) const {
    ik_f32 inv = 1.0f - d;
    return vec3d<T>(other.X * inv + X * d, other.Y * inv + Y * d,
                    other.Z * inv + Z * d);
  }

  //! Gets the Y and Z rotations of a vector.
  /** Thanks to Arras on the Irrlicht forums to 

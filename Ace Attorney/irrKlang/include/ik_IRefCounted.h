...
/** This class provides reference counting through the methods grab() and
drop(). It also is able to store a debug string for every instance of an object.
Most objects of irrKlang are derived from IRefCounted, and so they are reference
counted.
...
sound sources are managed by the engine (it will live as long as the sound
engine) and the documentation says so.
*/
class IRefCounted {
public:
  //! Constructor.
  IRefCounted() : ReferenceCounter(1) {}
  ...
  //!
  //! When you receive an object in irrKlang (for example an ISound using
  //! play2D() or play3D()), and you no longer need the object, you have to call
  //! drop(). This will destroy the object, if grab() was not called in another
  //! part of you program, because this part still needs the object. Note, that
  //! you only don't need to call drop() for all objects you receive, it will be
  //! explicitely noted in the documentation.
  //!
  //! A simple example:
  //!
  //! If you want to play a sound, you may want to call the method
  ...
  //! ISound* mysound = engine->play2D("foobar.mp3", false, false true);
  //! If you no longer need the sound interface, call mysound->drop(). The
  //! sound may still play on after this because the engine still has a
  //! reference to that sound, but you can be sure that it's memory will be
  //! released as soon the sound is no longer used.
